...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/amrgomezHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
tags= db.Table('tags', db.Column('SearchTerm_id',db.Integer,db.ForeignKey('SearchTerm.id')),db.Column('gifs_id',db.Integer, db.ForeignKey('gifs.id')))
...
class Gif(db.Model):
    ...
    def __repr__(self):
        "Gif title:{0}, URL:{1}".format(self.title,self.embedURL)
...
class PersonalGifCollection(db.Model):
    ...
    collection= db.relationship('Gif', secondary= user_collection, backref= db.backref('PersonalGifCollection', lazy='dynamic'),lazy='dynamic')
...
def get_gifs_from_giphy(search_string):
    ...
    baseurl="https://api.giphy.com/v1/gifs/search?apikey={0}&q={1}&limit=5".format(api_key, search_string)
    ...
    req= requests.get(baseurl).json()['data']
    return req
...
def get_or_create_gif(title, url):
    ...
    b= db.session.query(Gif).filter_by(title=title).first()
    if b:
        return b
    else:
        b=Gif(title=title, embedURL=url)
        db.session.add(b)
        db.session.commit()
        return b
...
def get_or_create_search_term(term):
    ...
    # This function should invoke the get_gifs_from_giphy function to get a list of gif data from Giphy.
        ...
        searchterm= SearchTerm(term=term,gifs=[])##ERROR##
        giphys = get_gifs_from_giphy(term)
        for y in giphys:
            t = get_or_create_gif(title=y['title'], url=y['embed_url'])
            searchterm.gifs.append(t)
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    gif_collection= PersonalGifCollection.query.filter_by(name=name,user_id=current_user.id).first()
    if gif_collection:
    ...
    # In other words, based on the input to this function, if there exists a collection with the input name, associated with the current user, then this function should return that PersonalGifCollection instance.
        ...
        gif_collection= PersonalGifCollection(name=name, user_id=current_user.id, collection=[])
        for z in gif_list:
            gif_collection.gifs.append(z)
...
def single_collection(id_num):
    ...
    gifs = collection.collection.all()
